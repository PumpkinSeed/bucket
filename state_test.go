...
func TestCreateFTSIndex(t *testing.T) {
	if err := createFullTextSearchIndex("order_fts_idx", true, "webshop", "product", "text"); err != nil {
		t.Error(err)
	}
}

func TestCreateEmptyIndexExpectError(t *testing.T) {
	if err := createFullTextSearchIndex("", true, "webshop", "token", "text"); err != ErrEmptyIndex {
